use crate::{parser::parse_bencode, RsbtBencodeError, TryFromBencode};
    ...
    ($type:ty,
            ...
            fn try_from(value: BencodeBlob) -> Result<Self, Self::Error> {
                ...
                for (key, value) in dictionary {
                    match key.as_str() {
                        ...
                        $($($failure_key => return Err(RsbtBencodeError::FailureReason(value.try_into()?)),)*)*
                        _ => (),
                    }
                }
        ...
        impl TryFrom<Vec<u8>> for $type {
            ...
            fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
                ...
                bencode.try_into().map_err(RsbtBencodeError::from)
            }
        }
